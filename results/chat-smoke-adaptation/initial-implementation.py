...
OUT=ROOT/'results/chat-smoke-adaptation';CACHE=ROOT/'experiments/.cache/chat-smoke-adaptation'
DEPTHS=[6,12,18,24]
...
def main():
    ...
    protocol={'common_sha256':hashlib.sha256((ROOT/'results/chat-smoke/protocol.json').read_bytes()).hexdigest(),'script_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'variant':variant,
       'seed':73,'rank':4,'alpha':8,'adapter_modules':'q_proj/v_proj in every retained block','steps':96,'batch':4,'lr_adapter':.0005,'lr_head':.001,'weight_decay':.01,'head_initialization':'train384-only frozen-feature linear classifier,200AdamWsteps lr.01 wd.1, inverse-frequencyclassweights',
       'checkpoints':[32,64,96],'selection':'Highest tune128 balancedaccuracy at targetdepth24(or12fixed); tie earlier. Other heads are diagnostic, no eval choice.',
    ...
    for d in DEPTHS:
        ...
        for _ in range(200):opt.zero_grad();torch.nn.functional.cross_entropy(head(x),labels['train'],weight=weights).backward();opt.step()
        heads[str(d)]=head
    del frozen
    ...
    for step in range(1,97):
        ...
        if step==2:
            gradient_check={'first_adapter_a':float(model.layers[0].self_attn.q_proj.a.grad.abs().sum()),'first_adapter_b':float(model.layers[0].self_attn.q_proj.b.grad.abs().sum()),'frozen_base_gradients':sum(p.grad is not None for p in model.parameters() if not p.requires_grad)}
            assert gradient_check['first_adapter_a']>0 and gradient_check['first_adapter_b']>0 and gradient_check['frozen_base_gradients']==0
        optimizer.step()
        if step%8==0:print(f'{variant}: step {step}/96 loss {float(loss.detach()):.4f} elapsed {time.perf_counter()-started:.1f}s',flush=True)
        if step in [32,64,96]:
