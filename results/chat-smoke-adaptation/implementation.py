...
def main():
    ...
    protocol={'common_sha256':hashlib.sha256((ROOT/'results/chat-smoke/protocol.json').read_bytes()).hexdigest(),'script_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'variant':variant,
       'seed':73,'rank':4,'alpha':8,'adapter_modules':'q_proj/v_proj in every retained block','steps':32,'batch':4,'lr_adapter':.0005,'lr_head':.001,'weight_decay':.01,'head_initialization':'train384-only frozen-feature linear classifier,200AdamWsteps lr.01 wd.1, inverse-frequencyclassweights',
