...
async function setup(){
 ...
 const start=performance.now();send('status',{text:'Loading the shared model and full-depth control (about 89 MB), tokenizers and runtime...'});
 ...
 for(const name of ['prefix','suffix','full']){const response=await fetch(new URL(name+'.onnx',base));if(!response.ok)throw Error(name+' download failed.');const bytes=await response.arrayBuffer();if(await digest(bytes)!==manifest.models[name].sha256)throw Error(name+' checksum mismatch.');sessions[name]=await ort.InferenceSession.create(bytes,{executionProviders:['wasm'],graphOptimizationLevel:'all'});}
 runtime={manifest,bt,qt,ort,sessions,setupMs:performance.now()-start};
 await classify('Hello, can you help me?',runtime,'full');await classify('Hello, can you help me?',runtime,'adaptive',true);return runtime;
}
...
onmessage=async({data:message})=>{try{
 const r=await setup();send('setup',{ms:r.setupMs});
 if(message.type==='classify'){if(typeof message.text!=='string'||!message.text.trim()||message.text.length>10000)throw Error('Enter between 1 and 10,000 characters.');const result=await classify(message.text,r,'adaptive');delete result.inputIds;send('classified',{result});return;}
