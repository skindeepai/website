...
async function runQwenModerationPass(prompt) {
    ...
    // Keep the moderation policy and explicit recent-chat context. Co-host memory is unrelated.
    const rendered = processor.apply_chat_template([
        { role: 'system', content: prompt.slice(0, split) },
        { role: 'user', content: prompt.slice(split + 2) }
...
self.addEventListener('message', async event => {
 ...
 try {
  ...
  if(m.mode==='baseline'||m.mode==='production') {
   ...
   self.postMessage({lab:true,id:m.id,result:{...result,...parse(result.text),ms:performance.now()-start,inputTokens:processor.tokenizer.encode(m.mode==='production'?processor.apply_chat_template([{role:'system',content:policy},{role:'user',content:suffix}],{tokenize:false,add_generation_prompt:true}):rendered).length}});return;
  }
