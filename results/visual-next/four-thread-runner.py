...
def run(threads=2):
    ...
    amendment = read(OUT / 'runtime-amendment.json') if threads == 4 else None
    if amendment:
    ...
    with torch.inference_mode():
        if amendment and not (OUT / 'thread-parity.json').exists():
            reference = traces[0]
            ...
            write(OUT / 'thread-parity.json', {'id': sample['id'], 'original_threads': 2, 'repeated_threads': threads, 'max_probability_delta': delta, 'max_coordinate_delta': point_delta, 'repeated_inference': repeated, 'status': 'passed'})
            print('Two/four-thread parity passed:', delta, point_delta, flush=True)
...
def summarize(protocol, traces):
    ...
    summary = {'protocol_sha256': sha(OUT / 'protocol.json'), 'samples': 50, 'present': 25, 'absent': 25, 'methods': methods,
        ...
        'runtime': {**protocol['runtime'], 'threads_used': sorted({r.get('threads', 2) for r in traces}), 'thread_amendment': 'runtime-amendment.json' if (OUT / 'runtime-amendment.json').exists() else None}, 'total_full_screen_ms': sum(r['elapsed_ms'] for r in traces),
