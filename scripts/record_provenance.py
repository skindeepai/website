...
def main():
    paths=[]
    for directory in ['scripts','experiments','results','content','docs','examples','archive','images']:
        paths.extend(p for p in (ROOT/directory).rglob('*') if p.is_file() and p.suffix in ['.py','.js','.cjs','.json','.html','.md','.txt','.png','.svg','.npz'] and p.name!='provenance.json' and not {'.cache','__pycache__'}.intersection(p.parts))
    paths.extend(ROOT/n for n in ['PLAN.md','EXPERIMENTS.md','README.md','lab.css','demo.css','style.css','results.css','sitemap.xml','.gitattributes'])
