...
def update(pages, legacy=None):
    ...
    tiny_model='<p><strong>Which tiny model?</strong> Google’s '+link('https://huggingface.co/google/bert_uncased_L-2_H-128_A-2','BERT miniature')+' with two layers and 4.37 million trained parameters. We fine-tuned it on 384 ToxicChat messages to return SAFE or BLOCK directly, without generating text. Uncertain messages go to a separate Qwen2.5-0.5B-Instruct classifier.</p>'+refs([('docs/chat-smoke-specialist.md','Architecture, training and model revision')])
    ...
    body=body.replace(sample(),sample()+tiny_model,1)
    body=body.replace('<p>Development examples select separate confidence limits for SAFE and BLOCK.', '<p>We train all BERT weights and a new two-output linear classifier. It averages the final token representations, excluding padding, then produces the SAFE/BLOCK scores.</p><p>In the cascade, a BLOCK score at or below 0.05 returns SAFE; at or above 0.8 returns BLOCK. Otherwise Qwen handles the message. These scores are not calibrated probabilities. When BERT runs alone, its BLOCK threshold is 0.3.</p><p>Development examples select separate confidence limits for SAFE and BLOCK.')
    ...
    body=flow(['Rate examples','Learn which settings matter','Score or adjust another example'],'The browser demo learns from settings behind drawings, rather than from image pixels.')+f'<p>{syn["math"]["browser_fixtures"]} browser calculation checks passed. In {syn["math"]["random_cases"]} generated bounded-edit cases, the solution agreed with an independent numerical solver.</p><p>On made-up preferences, the model learns simple patterns. It can fail when someone likes two separated styles or when a high predicted score does not match their real taste.</p><p><strong>We have not yet shown that people prefer its suggestions.</strong> The calculation tests verify the mechanics, not that human outcome.</p>'+refs([('demo.html','Try your own ratings'),('how-it-works.html','See how learning works')])+'<details><summary>Data and detailed tests</summary><p>These are generated numerical fixtures and synthetic preference rules, not a human-rating dataset.</p>'+refs([('docs/preference-followups.md','Actual sampler, contexts and constraints'),('results/synthetic/result.json','Five-seed learning results'),('results/synthetic/math-fixtures.json','Numerical fixtures'),('results-record.html#preferences','Original tables and comparisons'),('docs/method.md','Mathematics')])+'</details>'
    ...
    from refresh_faq import update as update_faq
    update_faq(pages)
