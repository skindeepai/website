 ...
 function start(type){
  ...
  if(!worker){try{worker=new Worker('scripts/shared-decision-worker.js');}catch(e){byId('shared-status').textContent=e.message;return;}
   worker.onmessage=({data})=>{
    ...
    if(data.type==='setup')byId('shared-setup').textContent='Setup this action: '+(data.incrementalMs/1000).toFixed(2)+' s; total '+(data.cumulativeMs/1000).toFixed(2)+' s ('+(data.adaptiveMs/1000).toFixed(2)+' s shared model'+(data.controlMs?'; '+(data.controlMs/1000).toFixed(2)+' s benchmark control':'')+'). Separate from processing.';
