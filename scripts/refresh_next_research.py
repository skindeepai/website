...
def update(pages):
    ...
    if (ROOT/'results/privacy-full-document/result.json').exists():
        full=read('results/privacy-full-document/result.json');before=full['methods']['prefix'];after=full['methods']['full'];total=full['person_occurrences']
        body='<p>Look for names that may need masking before sharing a document. The demo now checks the entire input; this is name detection, not complete anonymization.</p>'
        body+=table(['Same classifier','Complete name occurrences marked','Other tokens incorrectly marked'],[['First 1,200 tokens',f'{before["span_counts"]["fully_marked"]} / {total}',before['confusion']['fp']],['Entire input',f'{after["span_counts"]["fully_marked"]} / {total}',after['confusion']['fp']]])
        body+=f'<p>Reading the whole input found {full["new_complete_spans"]} additional complete name occurrences, while marking {after["confusion"]["fp"]-before["confusion"]["fp"]} more unrelated tokens. Some names are still missed or only partly marked.</p>'
        body+='<details><summary>What was tested?</summary><p>The same 50 TAB court documents, with the original trained weights and cutoff. Both rows count all annotated name occurrences, including the old unprocessed tail. The original model used 100 training and 25 development documents; this follow-up reuses the earlier test documents. All fit within the demo\u2019s 30,000-character limit.</p><p><a href="@/docs/privacy-full-document.md">Full-input comparison and every outcome</a> \u00b7 <a href="@/docs/practical-baselines.md#find-names-before-redacting-text">Original model and dataset</a></p></details>'
