...
def environment():
    ...
    return {
        ...
        'runtime_dependencies': {name: importlib.metadata.version(name) for name in (
            'torch', 'numpy', 'transformers', 'tokenizers', 'huggingface-hub', 'safetensors',
            'regex', 'requests', 'packaging', 'PyYAML', 'filelock', 'tqdm')},
        'transformers_declared_dependencies': distribution.requires,
        'transformers_import_path': str(package),
...
def main():
    ...
    parser.add_argument('--target-layer18', action='store_true')
    ...
    targeted = []
    if args.target_layer18:
        targeted = sorted((ident for ident, row in reference.items() if row['exit_layer'] == 18 and ident not in selected),
                          key=lambda ident: hashlib.sha256(('targeted-layer18-v1:'+ident).encode()).hexdigest())[:1]
    ...
    protocol = {
        ...
        'targeted_layer18_ids': targeted,
        'targeted_selection': 'Optional separate mechanism test: hash-selected from saved layer-18 exits. Outcome-dependent; not representative validation.',
    ...
    if args.runtime_wheel:
        env['runtime_wheel'] = {'filename': args.runtime_wheel.name, 'sha256': digest(args.runtime_wheel),
                                'scope': 'Published dependency replay; not recovery of the historical development build.'}
    ...
    with torch.inference_mode():
        for ident in selected + targeted:
            ...
            for adaptive in (False, True):
                ...
                records.append({'id': ident, 'path': 'early' if adaptive else 'full', 'label': source[ident]['label'],
                                'selection_group': 'targeted_layer18' if ident in targeted else 'hash_selected_24',
    ...
    result = {
        ...
        'targeted_mechanism_queries': len(targeted),
        'early_exit_counts': dict(Counter(row['depth'] for row in records if row['path'] == 'early' and row['selection_group'] == 'hash_selected_24')),
        'targeted_exit_counts': dict(Counter(row['depth'] for row in records if row['path'] == 'early' and row['selection_group'] == 'targeted_layer18')),
        ...
        'limits': ['Reviewer shares the same machine, model, source dataset and trained heads; not external independent replication.',
                   ...
                   'Only 24 predetermined queries plus any explicitly targeted mechanism case are rerun. Original full-dataset execution was not independently repeated.',
                   'The optional layer-18 example is selected using a known exit; it tests that code path, not generalization.',
