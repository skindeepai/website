"""Reconstruct the post-observation short-warmup study's initial readouts; no transformer inference."""
import os
...
OUT = ROOT / 'results/chat-smoke-reset'
...
def main():
    ...
    protocol = json.loads((OUT / 'joint-protocol.json').read_text())
    ...
    result = dict(scope='Reconstructed matched initial readouts using original cached frozen Qwen features; exploratory reused data, no new transformer inference.',
        recipe='Seed73, train384, per-depth train-only mean/std clamp.05, 896-to-2 linear, 5AdamW steps lr.01 weight_decay.1, inverse-frequency class weights normalized to mean1. No temperature scaling.',
        runtime=dict(torch=torch.__version__, numpy=np.__version__, threads=1),
        script_sha256=digest(Path(__file__)), parent_protocol_sha256=digest(OUT/'joint-protocol.json'),
        ...
        limitation='Adapter initialization runs at4threads; this independent reconstruction uses1thread. Check initial_tune parity before attributing differences. Post-observation repair baseline; same reused evaluation. Adds reporting without changing candidate training or selection.')
