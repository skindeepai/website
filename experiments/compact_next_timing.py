"""Actual stop/continue timing of the development-selected model on fresh50.
...
def main():
    study.guard('timing.json'); p,old,rows,qt,bt=study.load();fit=read(OUT/'fit.json');quality=read(OUT/'result.json')
    ...
    state=np.load(OUT/(key+'-gate.npz'));ids=p['splits']['fresh'][:50]
    ...
    protocol=dict(source_sha256=sha(__file__),quality_sha256=sha(OUT/'result.json'),ids=ids,paths=paths,passes=3,threads=2,
        ...
        selected=key)
    study.guard('timing-protocol.json');write('timing-protocol.json',protocol)
    ...
    write('timing-records.json',records);write('timing.json',dict(protocol_sha256=sha(OUT/'timing-protocol.json'),n=50,methods=methods,all_prediction_and_layer_traces_match=True))
