...
def receipts():
    ...
    out=begin('receipts',dict(dataset='240 numbered SROIE receipts from a contestant-corrected annotation mirror',revision=rev,
        ...
        target='Select total amount from provided text/boxes; OCR and image encoding excluded.',training='Fixed logistic regression C1 balancedclasses max_iter300. Weak candidate labels match gold total value. No threshold or test tuning.',
    ...
    for row in train:
        gold=float(re.sub(r'[^\d.]','',row['key']['total']))
    ...
    for row in test:
        ...
        baseline=max((a for a,_ in c),key=float,default=None);elapsed=time.perf_counter()-start
        gold_text=re.sub(r'[^\d.]','',row['key']['total']);gold=float(gold_text) if gold_text else None
        records.append(dict(id=row['id'],candidate_count=len(c),reference_available=gold is not None,gold_present=any(float(a)==gold for a,_ in c),
    ...
    write(out,'records.json',records);write(out,'result.json',dict(train_receipts=len(train),test_receipts=len(test),scorable_receipts=sum(r['reference_available'] for r in records),missing_reference=sum(not r['reference_available'] for r in records),merchant_groups=len(groups),
