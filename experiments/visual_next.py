...
def run(threads=2):
    ...
    amendment_name = 'runtime-amendment-12threads.json' if threads == 12 else 'runtime-amendment.json'
    amendment = read(OUT / amendment_name) if threads in [4, 12] else None
    parity_path = OUT / ('thread-parity-12threads.json' if threads == 12 else 'thread-parity.json')
    if amendment:
        assert sha(OUT / 'protocol.json') == amendment['protocol_sha256']
        assert sha(OUT / 'initial-runner.py') == protocol['source_sha256']['experiments/visual_next.py']
        assert sha(Path(__file__)) == amendment['amended_source_sha256']
    for source, checksum in protocol['source_sha256'].items():
        if amendment and source == 'experiments/visual_next.py':
            continue
    ...
    def forward(image, instruction):
        ...
        return {'points': {'max_patch': [(argmax % width + .5) / width, (argmax // width + .5) / height], 'connected_region': region_point(prob, width, height)}, 'peak_probability': float(prob.max()), 'patch_grid': [width, height], 'probabilities': prob.tolist(), 'layers_executed': list(layers), 'threads': threads, 'elapsed_ms': (time.perf_counter() - started) * 1000}

    traces = read(OUT / 'predictions.json') if (OUT / 'predictions.json').exists() else []
    with torch.inference_mode():
        if amendment and not parity_path.exists():
            reference = traces[0]
            sample = protocol['samples'][0]
            assert reference['id'] == sample['id']
            with Image.open(CACHE / sample['image_path']) as im:
                repeated = forward(im.convert('RGB'), sample['instruction'])
            delta = float(np.max(np.abs(np.asarray(reference['probabilities']) - np.asarray(repeated['probabilities']))))
            point_delta = max(abs(a - b) for key in repeated['points'] for a, b in zip(reference['points'][key], repeated['points'][key]))
            assert delta < 1e-4 and point_delta < 1e-5
            write(parity_path, {'id': sample['id'], 'original_threads': 2, 'repeated_threads': threads, 'max_probability_delta': delta, 'max_coordinate_delta': point_delta, 'repeated_inference': repeated, 'status': 'passed'})
            print('Thread-count parity passed:', threads, delta, point_delta, flush=True)
...
def summarize(protocol, traces):
    ...
    summary = {'protocol_sha256': sha(OUT / 'protocol.json'), 'samples': 50, 'present': 25, 'absent': 25, 'methods': methods,
        ...
        'runtime': {**{k: v for k, v in protocol['runtime'].items() if k != 'threads'}, 'threads_used': sorted({r.get('threads', 2) for r in traces}), 'thread_amendments': sorted(p.name for p in OUT.glob('runtime-amendment*.json'))}, 'total_full_screen_ms': sum(r['elapsed_ms'] for r in traces),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--threads', type=int, choices=[2, 4, 12], default=2)
    args = parser.parse_args()
    prepare() if args.prepare else run(args.threads)
